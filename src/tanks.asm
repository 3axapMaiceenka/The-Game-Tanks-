...
;assume bx points on shot, symbol and attribute are in si, df = 0
print_shot macro 
    mov ax, [bx + 2] ;load shot's y
    push bx
    mov bx, 80
    mul bx
    pop bx
    add ax, [bx + 4] ;add shots'x
    mov di, ax       
    shl di, 1 
    mov ax, si
    stosw
...
;delete tank with index in cx, assume bx points on the tank
...
endm
    ...
    .code     
main:
    ;set 80x25 16color text videomode
    mov ax, 03h 
    int 10h
    ...
    ;load videobuffer starting address
    mov ax, 0b800h
    mov es, ax
    
    ;get number of clock ticks in cx:dx
    mov ah, 00h
    int 1ah
...
game_loop:            
    ...
    mov dx, delay_timer  ;update approximately one time in 1/18 of second
    cmp dx, delay
    jl game_loop
    mov delay_timer, 0
    ...
    push player_y
    push player_frame
    call redirect ;the procedure checks entered keystrokes and changes tank's coordinate and current frame using passed arguemnts   
    pop player_frame
    pop player_y
    pop player_x 
...
proc print_map
    xor di, di        ; videobuffer offset
    mov cx, 80 * 25   ; number of symbols in the screen    
    mov ah, 0fh       ; symbol attribute
    mov bx, offset map
    cld    
print_loop:
    mov al, [bx]
...
;arguments: [bp + 4] = frame number; [bp + 6] = y; [bp + 8] = x, assume that symbol attribute is in ah
proc print_tank    
    push bp
    mov bp, sp    
    cld       
   
    ;load frame address in bx
    mov bx, [bp + 4]
    shl bx, 1
    mov si, frames[bx] 
    mov bx, 80
   
    ;load starting address to output tank in di
    push ax
    mov ax, [bp + 6]
    mul bx
    mov di, ax
...
;move tank, assume last key pressed is in al, arguments: [bp + 4] = frame number; [bp + 6] = y; [bp + 8] = x; return 1 in ax if 
;the tank has moved or changed direction 
proc redirect
    push bp
    mov bp, sp
    ...
    cmp al, 'w'
    jne cmp_a
    cmp bx, 0  ;if tank is ready to move up
    je move_up
    cmp bx, 2  ;if tank was dericted to down
    jne switch_to_up
    dec cx     ;will visually move down one cell without this
switch_to_up:
    xor bx, bx
    jmp redirect_check
move_up:
...
cmp_a:
    cmp al, 'a'
    jne cmp_s  
    cmp bx, 3 ;if tank is ready to move left
    je move_left
    cmp bx, 1 ;if tank was directed to right
    jne switch_to_left
    dec dx    ;will visually move right one cell without this
switch_to_left:
    mov bx, 3
    jmp redirect_check
move_left:
...
cmp_s:
    cmp al, 's'
    jne cmp_d
    cmp bx, 2 ;if tank is ready to move down
    je move_down
    cmp bx, 0 ;if tank was directed to up
    jne switch_to_down
    inc cx    ;will visually move up one cell without this
switch_to_down:
    mov bx, 2
    jmp redirect_check
move_down:
...
cmp_d:
    cmp al, 'd'
    jne cmp_f
    cmp bx, 1 ;if tank is ready to move right
    je move_right
    cmp bx, 3; if tank was directed to left
    jne switch_to_right
    inc dx
switch_to_right:
    mov bx, 1
...
;arguments: [bp + 4] = frame number; [bp + 6] = y; [bp + 8] = x
proc shoot
    push bp
    mov bp, sp
...
;check for collisions between shots, assume bx points on shot to check, si points on enemies or player's shots
;return 1 in ax if no colliions detected, arguments: [bp + 4] - shots count(player's or enemies)
...
;delete shot with index in cx, assume bx points on the shot, arguments: [bp + 4] - shots count 
proc shift_shots
    push bp
    mov bp, sp
...
;check for collision with hq, returns 1 in ax if no collision detected
...
;check if player was hit, return 1 in ax if wasn't, assume bx points in shot
...
;return index of tank which was hit, if no hit detected, return -1; arguments: [bp + 4] - shot's y, [bp + 6] - shot's x
...
;move shot and check for collisions with obstalces in map, return 1 in ax if shot continue moving, assume bx points on the shot
...
proc apply_bonus
    ...
    cmp cur_bonus, 0
    je limit_player_shots
    cmp cur_bonus, 1
    je decrease_enemies

increase_enemies:
    inc min_enemies_count
    jmp generate_new_bonus
decrease_enemies:    
    mov min_enemies_count, 2
    jmp generate_new_bonus
limit_player_shots:
    mov max_player_shots, 1
...
;assume that former bonus index is in bx
proc generate_bonus
    mov dx, seconds
    and dx, 1111b
    cmp dx, 4
...
correct_pos: 
    ...
    mov bx, ax
    shl bx, 1
    mov dx, bonus_positions[bx + 2]
    mov bonus_x, dx
    mov dx, bonus_positions[bx]
    mov bonus_y, dx
...
;check on collisions, return 1 in ax if movement is possible, arguments: [bp + 4] = frame number; [bp + 6] = y; [bp + 8] = x     
proc is_movement_possible
    ...
    ;check on collisions with walls
    ;load frame address in bx
    mov bx, [bp + 4]
    shl bx, 1
...
;check for collisions with other tanks; return 1 in ax if no collisions detected
;arguments: [bp + 4] = frame number; [bp + 6] = y; [bp + 8] = x     
...
;reads char without echo and without waiting to al, flushes keyboard buffer
proc get_char
    mov ah, 06h
    mov dl, 0ffh 
    int 21h         ;read char without echo and waiting to al
    push ax
    mov ah, 0ch
    xor al, al
...
;translates 16bit integer in ax into string, assume string address is in bx
proc itoa 
    xor cx, cx                      
    cmp ax, 0
...
;Prints string, assume starting address of the string is in ds:dx, changes the value of ah
print_string proc
     mov ah, 09h
     int 21h
     ret
...
proc update_time 
    ...
    jl update_bonus_change_timer
    mov allow_to_add_enemy, 1
    mov adding_delay_timer, 0
    
update_bonus_change_timer:
    inc bonus_change_timer
    cmp bonus_change_timer, bonus_change_delay
    jl set_shooting_delays
    mov bx, cur_bonus
    call generate_bonus
    mov bonus_change_timer, 0
...
proc print_hq      
    ;load starting address to output hq in di
    mov ax, hq_y
    mov bx, 80
    mul bx
    mov di, ax    
...
;player_shots[0] - first shot; player_shots[0] - first shot's direction, player_shots[2] - first shots's y coordinate
;player_shots[4] - first shots's x coordinate 
player_shots_count dw 0        
player_shots       dw max_player_shots * 3 dup (0)
 
;enemies[0] - first enemy, enemies[6] - second enemy
;enemies[0] - first enemy current frame, enemies[2] - first enemie y coordinate, enemies[4] - first enemie x coordinate
;hq - enemies headquarters, should be destroyed
...
bonuses             dw 09b2h, 0bb2h, 0db2h
cur_bonus           dw 0
bonus_x             dw 73
bonus_y             dw 1
bonus_positions     dw 1, 73, 2, 2, 23, 2
bonus_change_timer  dw 0
bonus_change_delay  equ 12
